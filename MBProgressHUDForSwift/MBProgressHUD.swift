...
enum MBProgressHUDMode: Int {
    case indeterminate = 0      //默认菊花
    case annularIndeterminate   //加载状态快速转动的圆环
    case determinate            //扇形加载进度
    case determinateHorizontalBar   //一条进度条
    case annularDeterminate     //圆环进度条
    case customView     //
    case text            //文本提示
}
    ...
    var progress: Float = 0.0 {
        didSet {
            if (indicator is MBRoundProgressView) {
                let indicatorView = indicator as! MBRoundProgressView
                indicatorView.progress = progress
            }
            if (indicator is MBBarProgressView) {
                let indicatorView = indicator as! MBBarProgressView
                indicatorView.progress = progress
            }
